...
try:
    ...
    # 3. 检查TabBar图标文件
    print("\n3. 检查TabBar图标文件...")
    stdin, stdout, stderr = client.exec_command(f"ls -la {remote_deploy_dir}/assets/ | grep -E 'tab|icon'")
    print("TabBar相关文件:")
    print(stdout.read().decode())
    
    # 4. 检查CSS文件
    print("\n4. 检查CSS文件...")
    stdin, stdout, stderr = client.exec_command(f"ls -la {remote_deploy_dir}/assets/ | grep -E '\\.css$'")
    print("CSS文件:")
    print(stdout.read().decode())
